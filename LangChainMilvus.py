...
def search_vectors(query, top_k=5):
    ...
    #获取检索结果的节点ID
    results_id = [res.entity.get('id') for res in results[0]]
    
    return results_id
def get_relevant_nodes(result_ids):
    # 你需要实现这个函数来根据结果 ID 获取相关节点
    # 例如，你可以根据 ID 从集合或其他数据源中获取相关文档
    return [texts[int(id_)] for id_ in result_ids]

query = "Llama2有多少参数?"
result_ids = search_vectors(query)
relevent_texts = get_relevant_nodes(result_ids)
print(relevent_texts)


llm = ChatOpenAI(model = "gpt-3.5-turbo")
qa = RetrievalQA(llm=llm,retriever=retriever)
response = qa.run("Llama2有多少参数?")
print(response) 
""" 
...
class MilvusRetriever(BaseModel):
    
    retriever: Milvus = Field(...)
